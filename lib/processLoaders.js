 ...
 */
const enrichResult = (result, originalImageInfo, imageOptions) => {
  ...
  const format = imageOptions.convert ? imageOptions.convert : originalImageInfo.format; // an array means it was not processed by the url-/file-loader and the result should still be an array
  // instead of a string. so in this case, append the additional export information to the array prototype

  if (Array.isArray(result)) {
    return `var res=${JSON.stringify(result)};res.width=${width};res.height=${height};res.format=${JSON.stringify(format || '')};module.exports = res;`;
  }
  ...
  return `${output}module.exports={src:src,width:${width},height:${height},format:${JSON.stringify(format || '')},toString:function(){return src;}};`;
};
/**
...
const replaceName = (name, originalImageInfo, imageOptions) => {
  return name.replace(/\[width\]/g, `${imageOptions.width || originalImageInfo.width}`).replace(/\[height\]/g, `${imageOptions.height || originalImageInfo.height}`);
};
/**
...
const convertToComponent = (image, originalImageInfo, imageOptions, loaderOptions) => {
  if (imageOptions.component === 'react') {
    ...
    const code = svgr.sync(image, loaderOptions.svgr || {}, {
    ...
    const transformed = babel.transformSync(code, {
      ...
      babelrc: false,
      configFile: false,
      presets: [babel.createConfigItem(require('@babel/preset-react'), {
        type: 'preset'
      }), // eslint-disable-line
...
const processLoaders = (context, image, originalImageInfo, imageOptions, loaderOptions) => {
  ...
  const furtherLoaderOptions = { ..._options.defaultFurtherLoaderOptions,
    ...loaderOptions,
    esModule: false
  }; // replace name

  furtherLoaderOptions.name = replaceName(furtherLoaderOptions.name, originalImageInfo, imageOptions); // change extension for converted images

  if (imageOptions.convert && furtherLoaderOptions.name) {
    furtherLoaderOptions.name = furtherLoaderOptions.name.indexOf('[ext]') >= 0 ? furtherLoaderOptions.name.replace('[ext]', imageOptions.convert) : furtherLoaderOptions.name += `.${imageOptions.convert}`;
  } // force inlining


  if (imageOptions.forceInline) {
    furtherLoaderOptions.limit = undefined; // force url
  } else if (imageOptions.forceUrl) {
    furtherLoaderOptions.limit = -1;
  } // build new loader context


  const furtherLoaderContext = { ...context,
    query: furtherLoaderOptions
  }; // get result of url-loader

  const result = _urlLoader.default.call(furtherLoaderContext, image);

  return enrichResult(result, originalImageInfo, imageOptions);
};

var _default = processLoaders;
